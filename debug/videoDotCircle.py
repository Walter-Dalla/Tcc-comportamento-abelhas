...
def add_rotating_point_to_frame(frame, t, width, height):
    ...
    angle = 2 * np.pi * t
    point_x = int(center_x + half_size * np.cos(angle))
    point_y = int(center_y + half_size * np.sin(angle))
...
def process_video(input_video_path, output_video_path):
    ...
    for frame_idx in range(frame_count):
        ...
        t = (frame_idx % fps) / fps
        frame = add_rotating_point_to_frame(frame, t, width, height)
        out.write(frame)
